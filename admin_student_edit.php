...
<?php
  ...
  if(isset($_GET['edit'])){
    $id  = $_GET['id'];
    $query8="SELECT * FROM siswa WHERE username = '$id'";
    $hasil8 = mysqli_query($conn, $query8) or die("Error");
    $sqle = mysqli_fetch_array($hasil8);
    $usernamed = $sqle[0]; 
    $passwordd = $sqle[1]; 
    $namad  = $sqle[2];
    $alamat_tetapd = $sqle[3];
    $alamat_sekarangd = $sqle[4];
    $tempat_lahird = $sqle[5];
    $tanggal_lahird = $sqle[6];
    $jenis_kelamind = $sqle[7];
    $agamad = $sqle[8];
    $nomor_hpd = $sqle[9];
    $emaild = $sqle[10];
  }
...
<form action="proses_ubah_data_siswa.php" method="post">
  <table border='0' style='border-collapse:collapse;' cellpadding='5' cellspacing='0'><input type='hidden' name='username' value='<?php echo"$usernamed"; ?>'>
    ...
    <tr>
      <td style="padding: 6px 10px;" valign='center'>ALAMAT TETAP : </td><td><input type="text" name="alamat_tetap" style='width:150px;' value='<?php echo"$alamat_tetapd"; ?>'></td>
    </tr>
    <tr>
      <td style="padding: 6px 10px;" valign='center'>ALAMAT SEKARANG : </td><td><input type="text" name="alamat_sekarang" style='width:150px;' value='<?php echo"$alamat_sekarangd"; ?>'></td>
    </tr>
    <tr>
      <td style="padding: 6px 10px;" valign='center'>TEMPAT LAHIR : </td><td><input type="text" name="tempat_lahir" style='width:150px;' value='<?php echo"$tempat_lahird"; ?>'></td>
    </tr>
    <tr>
      <td style="padding: 6px 10px;" valign='center'>TANGGAL LAHIR : </td><td><input type="text" name="tanggal_lahir" style='width:150px;' value='<?php echo"$tanggal_lahird"; ?>'></td>
    </tr>
    <tr>
